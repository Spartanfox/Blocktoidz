 ...
 * @author Ben
 */
public class CreditsScreen implements Screen, GestureListener , InputProcessor{
    Main main;
    SpriteBatch batch;
    Stage stage;
    ...
    public void LoadUI(){
        Gdx.app.log("Blocktoidz","Loading Credits screen");
        ...
        table.top();
        table.add(Styles.newImageButton("Title", "Title",null)).pad(10).padTop(AD_PADDING*4);
        table.row();
        table.add(Styles.newString("Credits",2)).center();
        table.row();
        table.add(Styles.newLabel("Programmer / Designer")).width(850).height(180);
        table.row();
        table.add(new Label("Ben Abbott",Styles.getSkin(),"big")).padBottom(15).center();
        table.row();
        table.add(Styles.newLabel("Theme Designer")).width(850).height(180);
        table.row();
        table.add(new Label("Samira Fateh",Styles.getSkin(),"big")).padBottom(15).center();
        
        table.row();
        table.add(Styles.newLabel("With thanks to")).width(850).height(180);
        table.row();
        table.add(new Label("Scott Dill",Styles.getSkin(),"big")).padBottom(12).center();
        table.row();
        table.add(new Label("Adaline Harris",Styles.getSkin(),"big")).padBottom(12).center();
        table.row();
        table.add(new Label("Raymond Buckley (Cloud UI Designer)",Styles.getSkin(),"big")).padBottom(12).center();
    ...
    public boolean touchDown(float x, float y, int pointer, int button) {
        ...
        int width  = Gdx.graphics.getBackBufferWidth ()/2;
        String sound = (x>width)? "fart" : "blop";
        Sounds.play(sound,Cal.map(Math.abs(y-height),0,height,0.5f,2f));
        return true;
    }
